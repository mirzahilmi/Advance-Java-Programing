...
public class GeometryDemo_v4_0 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < lingkarans.length; i++) {
            ...
            lingkarans[i] = new Circle(scx.nextDouble("  Masukkan radius lingkaran (m): "),
        ...
        for (Circle lingkaran : lingkarans) {
            System.out.printf(
                    "Lingkaran ke-%s:\nID\t\t: %s\nTitik asal\t: (%d, %d)\nRadius\t\t: %.2f m\nLuas\t\t: %.2f m2\nKeliling\t: %.2f m\n\n",
                    lingkaran.getID(), lingkaran.getID(), lingkaran.getXo(), lingkaran.getYo(), lingkaran.getRad(),
                    lingkaran.getArea(), lingkaran.getCircumference());
        }
