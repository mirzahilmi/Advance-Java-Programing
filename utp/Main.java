...
public class Main {
    public static void main(String[] args) {
        ...
        final String LOGIN_LOG = "Selamat datang pada laman login terminal sederhana!";
        final String OPTION = "1. Menampilkan semua antrian pada hari ini\n2. Menambahkan Antrian\n3. Log out dari Sistem";
        final String LOG = "Selamat datang pada aplikasi terminal sederhana!\nSilahkan pilih salah satu menu di bawah ini:\n"
                + OPTION + "\nMasukan Anda : ";
        ...
        Merchant.login(scx.nextLine("\nUsername\t: "), scx.nextLine("Password\t: "));
            ...
            switch (scx.nextLine(LOG)) {
                case "1":
                    ...
                    for (int i = 0; i < Merchant.queues.length; i++) {
                        ...
                        for (int j = 0; j < Merchant.queues[i].services.length; j++) {
                            ...
                            System.out.printf("\nLayanan Nomor : %d\nName : %s\nPrice : %d\n",
                                    Merchant.queues[i].services[i].key, Merchant.queues[i].services[i].name,
                ...
                case "2":
                    ...
                    for (int i = 0; i < Service.services.length; i++) {
                        System.out.println(BORDER_THIN);
                        System.out.printf("\nLayanan Nomor : %d\nName : %s\nPrice : %d\n",
                                Service.services[i].key, Service.services[i].name,
                                Service.services[i].price);
                        System.out.println(BORDER_THIN);
                    }

                    Merchant.addQueue(new Queue(scx.nextLine("Masukkan nama pemesan : "),
